    ...
    <>
      <main className="px-4 py-4 flex flex-col gap-2 min-h-[calc(100vh-48px)]">
        ...
        <div className="border-t border-t-oguogu-black pt-4 flex flex-col justify-start items-center gap-8">
          ...
          />
          {/* 데이터가 있는 경우 */}
          <OrderItem orderState="paymentCompleted" />
          <OrderItem orderState="preparingShipment" />
          <OrderItem orderState="inTransit" />
          <OrderItem orderState="delivered" />
          <OrderItem orderState="purchaseCompleted" />
          <OrderItem orderState="refundInProgress" />
          <OrderItem orderState="refundCompleted" />
        </div>
      </main>
    </>
  );
